<template>
      <div class = 'sidebarContainer'>
          ...
          <ul class = 'sidebar'>
            <li><router-link to='/myTreasure' ><i class = 'tiValue'></i><span @click="removeClass('tiValue')" class="dis">我的财富</span></router-link></li>
            <li><router-link to='/column' class ="domId"><i></i><span class="dis" @click="onClcik()">我的专栏</span></router-link></li>
            <li><router-link to='/collect'><i></i><span @click="removeClass()" class="dis">我的收藏</span></router-link></li>
            <li><router-link to='/comment'><i></i><span class="dis" @click="removeClass()">我的评论</span></router-link></li>
            <li><router-link to='/account_safe'><i></i><span class="dis" @click="removeClass()">账号与安全</span></router-link></li>
            <li  v-if="redDot"><router-link class="invite" to='/invite_stu'><i></i><span class="dis" id="invite" @click="removeClass(1)">邀请与收徒</span></router-link></li>
            <li v-else><router-link class="invite" to='/invite_stu'><i></i><span class="dis"  @click="removeClass(1)">邀请与收徒</span></router-link></li>
...
export default {
  ...
  methods: {
    ...
    removeClass(str){ // 在页面中获得url,如果还在邀新页面，标识不消失
        ...
        tiValue_i.style.background = str == 'tiValue' ? "transparent" :"transparent";
    },
    onClcik(){
        localStorage.setItem('award_approve',1)
        localStorage.setItem('trader_pwd',1)
        ...
        var domId =document.getElementsByClassName("domId");
          var domX =domId[0].children[0];
          if(domId[0]!=undefined){
             domX.style.cssText='display: inline-block;background:transparent;width:2px;';
          }
                  const tiValue_i = document.getElementsByClassName("tiValue")[0];
           tiValue_i.style.background = "transparent";
    },
    Quit(){
...
/*     let url = window.location.href;
    const shiftIn = url.indexOf('shift_in');
    const tiValue_i = document.getElementsByClassName("tiValue")[0];
    tiValue_i.style.background = shiftIn != '-1' ?"#f00" : "transparent";*/
      //请求用户信息
        const that = this;
          const list = Lib.M.dataRequest({
                      "userId":localStorage.getItem('kg_desk_userId')
          ...
          Lib.M.ajax({
              ...
              'success':function (data){
                  if(data.code == 10000){
                    ...
                    that.personInfo.avatar =data.userRole == 1 ? data.avatar:data.columnAvatar;
  ...
  watch: {
    // 如果路由有变化，会再次执行该方法
     $route(to) {
            if (to.path.indexOf('invite_stu') != -1) {
              var domId =document.getElementsByClassName('invite');
              domId[0].className = 'invite router-link-exact-active'; //在原来的后面加这个
            }
            else {
              var domId =document.getElementsByClassName('invite');
              domId[0].className = 'invite'; //移除
            }
        }
  }
}
