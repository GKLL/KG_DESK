                  ...
                  <div class = 'myColumn_main'>
                    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
                        <el-form-item class = 'ml20' label="视频标题" prop="title">
                          <el-input  size = 'small' v-model="ruleForm.title" placeholder = '请输入视频标题'></el-input>
                        </el-form-item>
                        ...
                        <el-form-item class = 'ml20' prop = 'videoInfo' style = "margin-bottom:20px">
                            <el-upload
                             v-show = "videoType == 11"
                              ...
                              :file-list="fileList"
                              >
                              <el-button :loading = "limitUpload" size="small" type="primary">{{videoInfo == '' ? '添加视频':'重新上传'}}</el-button>
                              <div slot="tip" class="el-upload__tip color999">限200M以内mp4格式视频</div>
                            </el-upload>
                            <div v-show = "videoType == 22">
                              <el-input  size = 'small' v-model="videoInfo2" placeholder = '请粘贴外部视频网站的视频通用代码' @change = 'changeVideoInfo2'></el-input>
                        ...
                        <div class = 'pl20'>
                                      ...
                                      <el-form-item label="封面图" prop="listCoverImgPost2">
                                          <div class = 'showDialog cursorPointer'>
                                                <img v-if = "listCoverImgPost2 == null" class = 'cursorPointer listCoverImg' src = "../../../assets/img/xxx.png" />
                                                <img v-else class = 'cursorPointer listCoverShow' :src = "listCoverImgShow2"  />
                                      ...
                                      <div v-if = "ruleForm.articleType == '' ? false : true">
                                        <el-form-item  v-if = "ruleForm.articleType == 1 ? false : true" label="视频来源" prop = "articleSource">
                                          <el-input size = 'small' v-model="ruleForm.articleSource" placeholder = '请输入'></el-input>
                                        </el-form-item>
                                        <el-form-item  v-if = "ruleForm.articleType == 1 ? false : true" label="原视频链接" prop = "articleLink">
                                         <el-input size = 'small' v-model="ruleForm.articleLink" placeholder = '请输入' ></el-input>
                                        </el-form-item>
                                      </div>
                                      <el-form-item label="打赏" prop = "bonusStatus">
                                              <el-radio-group v-model = "ruleForm.bonusStatus" >
                                                <el-radio label="3">
                                                  开启                                   
                                                  <span style = "color: #999" class = 'marlr10'>                                                                   注：开启打赏后，可在视频详情页展示打赏入口，其他用户可以向您打赏一定额度的钛值
                                                  </span>
                                                </el-radio><br/>
                                                <el-radio label="4" class = 'martop10'>
                                                    不开启
    ...
    data () {
     var validateTitle = (rule,value,callback) =>{
      ...
      var validateCoverImg = (rule, value, callback) => {
        if(this.listCoverImgPost2== null){
      ...
      var validateVideo= (rule, value, callback) => {
        if(this.videoInfo == '' && this.videoInfo2 == ''){
      ...
      return {
        ...
        rules: {
          ...
          articleSource:[
            ...
            { required: true, message: '请输入原视频链接', trigger: 'blur' },
...
/*          autoCropWidth: 220 ,
    ...
    mounted() {
      ...
      const that = this;
      this.articleId = localStorage.getItem('editor_articleId')
        //获取视频详情
        const editor_list = Lib.M.dataRequest({"articleId":this.articleId})
            Lib.M.ajax({
                  'type':'post',
                  ...
                  'success':function (data){
                    if(data.code == 10000){
                        ...
                        that.ruleForm = data;
                        that.listCoverImgShow2= data.articleImage == null ? '' :Lib.C.imgUrl + data.articleImage;
                        that.ruleForm.articleType = data.articleType == null ? '' :data.articleType + '';
                        // that.ruleForm.articleType = data.bonusStatus == null ? '' :data.bonusStatus + '';
                        // that.coverImg.push(Lib.C.imgUrl + data.articleImage)
                        that.ruleForm.title = data.articleTitle;
                        that.ruleForm.textarea = data.articleDescription;
                        that.listCoverImgPost2 = data.articleImage;
                        that.ruleForm.bonusStatus = data.bonusStatus == 1 ? '3' : '4';
                        that.textnum = data.textnum;
                        //判断视频是本地还是连接
/*                        if(data.videoFilename != null && data.videoUrl != '' && data.videoFilename != '' && data.videoUrl != null){
                            that.fileList =[ {name: data.videoFilename, url: data.videoUrl}]
                            that.videoInfo = data.videoUrl
                            that.videoType = '11';
                        }else{
                            that.videoType = '22';
                            that.videoInfo2 = data.videoUrl
                            that.fileList = []  
                        }*/
                        if((data.videoFilename == null || data.videoFilename == '') && data.videoUrl != ''){
                            that.videoType = '22';
                            that.videoInfo2 = data.videoUrl
                            that.fileList = []        
                        }else if((data.videoFilename != '' || data.videoFilename == '') && data.videoUrl == ''){
                            that.videoInfo = data.videoUrl
                            that.videoType = '11';
                            that.fileList = [] 
                        }else{
                        ...
                        if(data.listAllBonus != null){
                           that.award_list = data.listAllBonus;
                         }else{
                          that.award_list = null
                         }
                       

                        //获取视频封面图
                        // that.coverImg.push({"img":Lib.C.imgUrl + data.articleImage})
                        //获取视频内容
                        that.editorContentHtml = data.articleText;
                        that.editorContentText = data.articleDescription;
                        that.content = data.articleText;
                        //获取视频标签
                        let tagArr = [];
                        if(data.articleTagnames != null){
                          tagArr = data.articleTagnames.split(',')
                          for(var i in tagArr){
                            that.tag[i].value = tagArr[i]
                          }
                        }
                    }else{
                        that.$message({
    ...
    methods:{
      ...
      handleSuccess(response, file, fileList){
        if(response.errorCode == 10000){
                  this.videoInfo = response.data[0].filePath
      ...
      var fileList=e.target.files[0]
            ...
            let config = {
                ...
                if (res.errorCode == 10000) {
                    that.videoInfo = res.data[0].filePath;    
                }else{
                      that.$message({
                        type:"error",
           ...
           axios.post(
            Lib.C.uploadImgUrl,
             ...
             that.busying = null;  
                res =res.data; 
                if (res.errorCode == 10000) {
                    that.listCoverImgShow2 = Lib.C.imgUrl + res.data[0].filePath;
                    that.listCoverImgPost2 = res.data[0].filePath;
      ...
      changeVideoInfo2(){
               if(this.videoInfo2 != ''){
                  if(this.videoInfo2.indexOf('src') != '-1') {
                    this.videoInfo2 = this.videoInfo2.replace(new RegExp("'","gm"),'"');
                    this.videoInfo2 = 'https://'+this.videoInfo2.match(/:\/\/(\S*)"/)[1];
                  }else if(this.videoInfo2.indexOf('http') != '-1' && this.videoInfo2.indexOf('src') == '-1'){
                      this.videoInfo2 = 'https://'+this.videoInfo2.match(/:\/\/(\S*)/)[1];
                  }
        }
      },
         ...
         this.tagStr =arr.join(',')
            ...
            let autoSave_list= {
                      //发布基础信息
                       ...
                       "articleImage":this.listCoverImgPost2,
                       "articleDescription":this.ruleForm.textarea,
                       "articleTagnames":this.tagStr,
                       "articleType":this.ruleForm.articleType,
                       "articleFrom":1,
                       "bonusStatus":this.ruleForm.bonusStatus == '3' ? '1' : '0',
                       "publishStatus":4,
                       "publishKind":2,
                       'videoFilename':this.videoType == '11' ? this.ruleForm.videoFilename : "",
            };
            if(this.videoType == '11'){
              if(this.videoInfo == ''){
                 autoSave_list.videoUrl = '';
              }else if(this.videoInfo != '' && this.videoInfo.indexOf('oss') != '-1'){
                  autoSave_list.videoUrl = this.videoInfo
              }else{
                autoSave_list.videoUrl =Lib.C.videoUrl +  this.videoInfo
              }
            }else{
              autoSave_list.videoUrl = this.videoInfo2;
            }
           
/*            if(this.videoType == '11'){
              autoSave_list.videoFilename = this.videoType == '11' ? this.ruleForm.videoFilename : "",
            }*/
      ...
      onPreview(){
        ...
        obj.articleTitle = this.ruleForm.title;
        obj.articleDescription = this.ruleForm.textarea;
        obj.publishKind = 2;
        obj.videoFilename = this.videoType == '11' ? this.ruleForm.videoFilename : "";
        if(this.videoType == '11' && this.videoInfo !=''){
          if(this.videoInfo.indexOf('http') == '-1' && this.videoInfo.indexOf('src') == '-1'){
            obj.videoUrl =  Lib.C.videoUrl + this.videoInfo
          }else{
            obj.videoUrl =this.videoInfo
          }
          
        }else if(this.videoType == '11'  && this.videoInfo =='') {
          obj.videoUrl = ''
        }else if(this.videoType == '22'){
          obj.videoUrl = this.videoInfo2
        }
        localStorage.setItem('preview_data',JSON.stringify(obj))
      ...
      saveDraft(){
       if(this.videoInfo2 != ''){
                  if(this.videoInfo2.indexOf('src') != '-1'  && this.videoInfo2.indexOf('://') != '-1') {
                    this.videoInfo2 = this.videoInfo2.replace(new RegExp("'","gm"),'"');
                    this.videoInfo2 = 'https://'+this.videoInfo2.match(/:\/\/(\S*)"/)[1];
                  }else if(this.videoInfo2.indexOf('http') != '-1' && this.videoInfo2.indexOf('src') == '-1'){
                      this.videoInfo2 = 'https://'+this.videoInfo2.match(/:\/\/(\S*)/)[1];
         ...
         this.tagStr =arr.join(',')
            let saveDraft_list= {
                      //发布基础信息
                       ...
                       "publishStatus":4,
                       "publishKind":2,
                       // "videoUrl":this.videoType == '11'? Lib.C.videoUrl +  this.videoInfo : this.videoInfo2,
                       'videoFilename':this.videoType == '11' ? this.ruleForm.videoFilename : "",
            ...
            if(this.videoType == '11'){
              if(this.videoInfo == ''){
                 saveDraft_list.videoUrl = '';
                  ...
                  saveDraft_list.videoUrl = this.videoInfo
              }else{
                saveDraft_list.videoUrl =Lib.C.videoUrl +  this.videoInfo
              }
            }else{
              saveDraft_list.videoUrl = this.videoInfo2;
            }
/*            if(this.videoType == '11'){
      ...
      //发布视频
      submit(formName){
        console.log('this.videoInfo2',this.videoInfo2)
       if(this.videoInfo2 != ''){
                  if(this.videoInfo2.indexOf('src') != '-1'  && this.videoInfo2.indexOf('://') != '-1') {
                    this.videoInfo2 = this.videoInfo2.replace(new RegExp("'","gm"),'"');
                    this.videoInfo2 = 'https://'+this.videoInfo2.match(/:\/\/(\S*)"/)[1];
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            if(this.videoType == '11'){
              if(this.videoInfo == ''){
                 publish_list.videoUrl = '';
              }else if(this.videoInfo != '' && this.videoInfo.indexOf('oss') != '-1'){
                  publish_list.videoUrl = this.videoInfo
              }else{
                publish_list.videoUrl =Lib.C.videoUrl +  this.videoInfo
              }
            }else{
              publish_list.videoUrl = this.videoInfo2;
            }
/*            if(this.videoType == '11'){
