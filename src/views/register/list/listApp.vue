  ...
  <el-form :model="ruleForm"  :rules="rules" ref="ruleForm"  class="demo-ruleForm loginForm">
          ...
          <el-form-item label="" prop="phone" class = 'phoneInput'>
               <el-input   size = 'small' placeholder="输入手机号" type="text" v-model="ruleForm.phone" auto-complete="off" >
                ...
                <div>
                   <div class="ln">
                     <div id="moblie_registerBlock"></div>
                   </div>
                   <input type='hidden' id='csessionid02' name='csessionid02'/>
                   <input type='hidden' id='sig02' name='sig02'/>
                   <input type='hidden' id='token02' name='token02'/>
                   <input type='hidden' id='scene02' name='scene02'/>
                </div>
...
<!--           <el-form-item label="" prop = 'inviteCode'>
 <el-input   size = 'small' v-model="ruleForm.inviteCode" placeholder="邀请码" ></el-input>
          ...
          <p class ='color999 agree'>
            <span>注册即同意</span>
            <span @click = 'toAbout_register'>《千氪财经用户协议》</span>
            <!-- href = '../about_register/list.html' -->
          </p>
          <el-form-item class = 'register_formItem'>
            <el-button  size = 'small' type="primary" :loading = "loading" @click="submitForm('ruleForm')" class="btn width100" >完成</el-button>
          </el-form-item>
...
data() {
  ...
  mounted(){
        this.areaCode = codeData;
         Lib.M.aliBlock('#moblie_registerBlock',this,'csessionid02','sig02','token02','scene02')
  },
  //相关操作事件
  methods: {
                    ...
                    me.interval = window.setInterval(function() {
                        if ((me.time--) <= 0) {
                                   me.time = 90;
                                    ...
                                    Lib.M.aliBlock('#moblie_registerBlock',me,'csessionid02','sig02','token02','scene02')
                                    me.handleSlide('','','','')
                        }
                    }, 1000);
                Lib.M.loginAjax({
    ...
    this.ruleForm.inviteCode =Lib.M.getUrlQuery('inviteCode');
  }
}
</script>
<style  lang="less">
