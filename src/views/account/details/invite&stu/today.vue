<template>
  <div class="today">
      <el-table
        ...
        style="width: 100%"
        @sort-change="sortBy"
        :default-sort = "{prop: 'todayIncome', order: 'descending'}"
        ...
        width="100">
            <template slot-scope="scope">
                <div :class="'star-six'+scope.row.rank"><span>{{ scope.row.rank }}</span></div>
            </template>
        </el-table-column>
        <el-table-column
        prop="name"
    ...
    >
          <div>
            ...
            <el-input @focus="customPay"  v-model="payInput" :value="payInput"  placeholder="自定义打赏额度"></el-input>
      ...
      <div class="mask" v-if="payVisible">
        <div class="withdrawmask_info">
          <p><span>确定打赏Ta {{payInput != 0 ? payInput : rewardStuParams.rewardTv}}TV吗？</span><i class="el-icon-circle-close-outline" @click="payVisible=!payVisible;rewardStuParams.txPassword='';rewardStuParams.rewardTv='';payInput=''"></i></p>
  ...
  export default {
    ...
    data() {
      return {
        params: {
             userId: '',
             columnType: 0, // 今日进贡 默认按照进贡数量的降序排
            //  orderByTime:1,
